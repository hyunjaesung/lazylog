...
전혀 두렵지 않다. 몇 개의 분기만 지우면 끝이다. 하위 컴포넌트는 받은 데이터를 그대로 렌더링하는 단일 책임만 수행하고, 모든 분기 처리는 최상단 컴포넌트 하나에서만 관리된다. `Page`는 도메인 로직을 처리하는 역할을 수행하고 있고 다른 컴포넌트 들은 UI를 그리는 역할을 수행하고 있다. 관심사를 분리하고 역할에 맞게 동작하고 있다.
...
처음엔 잘 동작합니다. 하지만, 예상대로 정책은 곧 바뀐다.
...
**😨 Props는 비대해지고, 컴포넌트는 복잡해졌다.** 정책이 추가될수록 `Profile`은 점점 비대한 컴포넌트가 됩니다. 처음 우리가 잘 설계해두었던 "재사용 가능한 단순 컴포넌트"는 사라지고, **도메인 정책을 품은 거대한 UI 덩어리**가 된다.
...
이런 경우 **제어 역전**을 고려해볼 수 있다. 이름은 거창하지만 단순하다.
