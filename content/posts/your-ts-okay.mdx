...
TypeScript는 강력한 타입 시스템을 제공하지만, 생산성을 위해 다양한 예외를 두고 있어서 완벽한 타입 검증을 하지 않는다. 이변성, 옵셔널 프로퍼티, 함수 타입의 공변·반공변 관계로 인해 예상치 못한 런타임 에러가 발생할 수 있다. 따라서 TypeScript를 신뢰하되, 타입 시스템의 한계를 이해하고 조심스럽게 사용해야 한다.
