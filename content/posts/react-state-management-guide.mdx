...
리액트 상태는 크게는 범위로 나눠볼 수 있고 세세하게 나눠 보자면 관심사의 측면에서 다시 나눠볼 수 있다. **범위**의 측면에서 본다면 몇몇 컴포넌트에 국한되서 영향을 주는 **로컬 상태**와 프로젝트 전체에 영향을 줄 수 있는 **전역 상태**로 나뉜다. **전역 상태**는 다시 관심사에 측면에 따라서 UI에 영향을 주는 **UI 상태**와 서버에서 영향을 받는 **캐싱된 서버 상태**로 나눠 볼 수 있다. 이에 따라 상태 관리 하는 법들을 생각해보자
