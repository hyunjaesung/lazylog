---
title: 자바스크립트 구 버전 브라우저 대응 트러플 슈팅
date: 2025-03-18
category: Tech
...
이러한 문제를 방지하기 위해서는 개발 환경(`esbuild.target`)과 프로덕션 환경(`build.target`)의 타겟 버전을 동일하게 설정하면 된다. 다만, 개발 환경에서 기본값(`esnext`) 대신 낮은 버전(`es2015` 등)을 설정할 경우, ESBuild가 문법 변환 작업을 수행해야 하므로 개발 서버의 속도가 약간 느려질 수 있다.
...
Chrome 79, Safari 14를 지원 범위로 잡았지만 Vite의 build.target은 보수적인 접근으로 둘 중 구버전을 기준으로 트랜스파일을 하게된다.

아래의 예시는 Chrome 64, Safari 15를 타겟으로 했을때 예시이다. Chrome 64는 ES8(2017) 까지 지원하고 Safari 15는 ES2021까지 지원하기 때문에 옵셔널 문법을 허용한다. 실제로 빌드해보면 아래와 같이 둘 중 더 낮은 버전의 자바스크립트 문법으로 변환된다.
...
```jsx
// chrome64을 타겟으로 react.production.js 에서 실험
...
// 변환 후
function fl(s){const T={test:"stevytest"};return console.log(T==null?void 0:T.test)
```
